#!/usr/bin/env python3
"""Fit one physical-corner shape across independent grids and domains."""
...
seed=np.load("results/high_order_physical_corner_shared_domain_solve_state.npz")["coefficients"]
...
payload={
    "status":"shared_grid_domain_nonlinear_pilot_passes_0.025" if result["converged"] else "shared_grid_domain_nonlinear_pilot_does_not_yet_pass_0.025",
    ...
    "limitations":[
        "four independent high-order fold cases",
...
Path("results/high_order_physical_corner_shared_grid_domain_solve.json").write_text(
...
np.savez_compressed("results/high_order_physical_corner_shared_grid_domain_solve_state.npz",**archive)
