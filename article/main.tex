...
\hypersetup{
  ...
  pdftitle={From a Free-Boundary MOTS Saddle-Node to a Spacelike Marginal Tube in Dynamical Five-Dimensional Gravity},
  pdfauthor={Ronald Bibb},
  pdfsubject={Critical formation and post-fold development of a free-boundary marginally outer trapped tube},
  pdfkeywords={braneworld; marginally outer trapped surface; free boundary; saddle-node bifurcation; dynamical horizon; numerical relativity; Goldberger-Wise stabilization}
}
...
\title{From a Free-Boundary MOTS Saddle-Node to a Spacelike Marginal Tube
in Dynamical Five-Dimensional Gravity}
\author{Ronald Bibb\\
\small Independent Researcher, Lilburn, Georgia, USA\\
\small \texttt{ronbibb@gmail.com}}
\date{August 28, 2026}

\maketitle

\begin{abstract}
Pair creation is a local statement about marginally outer trapped surfaces
(MOTSs); its physical significance depends on what the stable branch becomes.
We resolve both the critical creation and the subsequent geometric development
of brane-terminating MOTSs in the $SO(3)$-symmetric sector of a dynamically
evolved, Goldberger--Wise-stabilized five-dimensional spacetime.  An
independently constructed parent sequence contains no accepted capped MOTS at
$t/\ell=0.001$ and exactly two at $t/\ell=0.0015$ on three successive
spacetime grids.  Pseudo-arclength continuation locates a common critical time
$t_*/\ell\simeq1.17254\times10^{-3}$ in the chosen foliation.  The principal
stability eigenvalue crosses zero through a real sign-definite mode; discrete
adjoint projections give nonzero transversality and quadratic coefficients;
and the invariant one-sided proper-area separation follows the predicted
square-root law.  An independent G10 half-timestep evolution reproduces the
critical bracket and gives a free exponent $p\simeq0.498$.  We then follow the
stable outer branch over nine evenly spaced matched leaves spanning
$0.00121875\leq t/\ell\leq0.00146875$, with physical leaf spacing
$3.125\times10^{-5}\ell$.  It forms a resolved spacelike
marginal tube with negative inward expansion, strictly increasing area,
positive principal stability, and orthogonal brane contact.  Its geometry and
causal signature transfer across G9/G10/G11 and survive the half-timestep
evolution.  A brane-inclusive reflected quasi-local charge ledger closes over
a finite segment with a worst charge--flux discrepancy of $0.302\%$, robust
under three spatial stencils.  The combined result traces a boundary-created
stable branch from its saddle-node through a finite-resolution
free-boundary dynamical-horizon segment.  It remains quasi-local and
foliation-dependent and is neither a continuum theorem nor an event-horizon,
global-topology, or source-ownership reconstruction.
\end{abstract}
...
The fold mechanism itself is therefore not new.  The first question here is
whether its hypotheses are realized when the marginal surface has a physical
boundary, the contact condition enters the operator domain, and the ambient
geometry is supplied by a nonlinear evolution rather than a prescribed metric
family.  The second is physical rather than classificatory: if a stable outer
branch is created, does it persist as a coherent marginal tube with the sign,
causal character, area evolution, and balance behavior expected of a
dynamical horizon?  A spacelike hypersurface foliated by marginal leaves with
negative inward expansion is the quasi-local dynamical-horizon structure that
captures black-hole growth without reconstructing an event horizon
\cite{AshtekarKrishnan2002,AshtekarKrishnan2003}.

We answer both questions for brane-terminating free-boundary MOTSs in a
compact, explicitly stabilized braneworld.  The spacetime is generated by a
nonlinear five-dimensional initial-boundary-value evolution; the stability
operator is non-self-adjoint; and its domain contains the Robin condition
induced by the brane geometry.  Existing closed-MOTS bifurcation examples do
not contain these features, while existing free-boundary stability and
rigidity results do not guarantee dynamical pair creation or the post-fold
development of a spacelike tube.  The contribution is thus neither a new
universal bifurcation class nor pair detection in another metric.  It is a
controlled numerical connection between a boundary-sensitive fold and the
subsequent marginal geometry selected by its stable branch.

Direct three-grid formation, pseudo-arclength continuation, critical-mode and
discrete-adjoint tests, invariant square-root scaling, and an independent G10
half-timestep evolution establish the local saddle-node at finite resolution.
The stable outer branch is then followed over a fixed post-fold interval.
Negative inward expansion, increasing area, positive principal stability, and
a uniformly spacelike tube tangent persist under G9/G10/G11 spatial transfer
and the half-timestep control.  A brane-inclusive local and finite-segment
quasi-local balance supplies an independent geometric check.  Together these
results support a finite-resolution free-boundary dynamical-horizon segment in
the imposed $SO(3)$-symmetric sector and chosen foliation.  The analysis does
not establish an event horizon, global topology, source ownership, the
nonsymmetric nonprincipal spectrum, or a continuum theorem.
...
\subsection{Post-fold marginal-tube observables}

Let $u^a$ be the future unit normal to a spatial slice, $s^a$ the outward
unit normal to a marginal leaf, and
$\ell^a=(u^a+s^a)/\sqrt2$, $n^a=(u^a-s^a)/\sqrt2$.  The normal projection of
the positive-time tube generator is written
\begin{equation}
X^a=A\ell^a-Bn^a,
\qquad X^2=2AB.
\label{eq:tube-generator}
\end{equation}
A future-outward spacelike segment requires $A>0$ and $B>0$, not merely
$X^2>0$.  On a marginal leaf $\theta_{(\ell)}=0$, while
$\theta_{(n)}<0$ supplies the future trapped sign.  If the full deformation is
$\mathcal X^a=X^a+V^a$, with $V^a$ tangent to the leaf, orthogonal brane
contact and endpoint motion within the brane give
$V\mathbin\cdot\nu=0$.  The one-sided area law is therefore
\begin{equation}
\frac{\dd\mathcal A}{\dd\lambda}
=-\int_{\mathcal S}B\theta_{(n)}\,\dd A>0.
\label{eq:tube-area-law}
\end{equation}

The physical cap is open, so its intrinsic-curvature balance is evaluated on
the reflected three-leaf.  The resulting AdS-subtracted quasi-local charge
candidate $Q_{\rm AdS}$ contains a coupled cap--brane seam contribution fixed
by the Israel geometry.  The finite-segment test has the form
\begin{equation}
Q_{\rm AdS}(\lambda_2)-Q_{\rm AdS}(\lambda_1)
=\int_{\lambda_1}^{\lambda_2}F_{\rm bulk+brane}\,\dd\lambda.
\label{eq:tube-balance}
\end{equation}
This is a quasi-local balance on the tested tube segment, not a global mass or
source-ownership ledger.  The boundary variation, charge convention, and
separate spatial and temporal tests are given in the Supplemental Material
...
\subsection{Post-fold development of the stable branch}

The fold calculation alone does not show that neighboring outer MOTSs belong
to a physically coherent tube.  We therefore follow the accepted stable
branch over the fixed interval
$0.00121875\leq t/\ell\leq0.00146875$.  At each time the surface profile,
principal stability eigenvalue, both null expansions, and the normal
projection of the tube generator are reconstructed independently.  Backward,
centered, and forward velocity paths are retained so that a positive centered
norm is accepted as spacelike only when it is separated from the one-sided
spread.  The resulting tests are summarized in
Table~\ref{tab:tube-controls}.  They directly evaluate the oriented generator
and area law in Eqs.~\eqref{eq:tube-generator}--\eqref{eq:tube-area-law} and
the finite-segment balance in Eq.~\eqref{eq:tube-balance}.

\begin{table}[t]
\centering
\caption{Post-fold marginal-tube controls.  Relative differences are maxima
over the stated comparison unless otherwise noted.}
\label{tab:tube-controls}
\begin{tabular}{@{}p{0.30\linewidth}p{0.25\linewidth}p{0.35\linewidth}@{}}
\toprule
risk & criterion & outcome \\
\midrule
grid-dependent tube & G9/G10/G11 geometry $<1\%$ & maximum geometry difference $1.24\times10^{-6}$; stability signs agree \\
unresolved causal character & every surface node spacelike & all 501 nodes on every tested interior leaf resolve spacelike \\
temporal bias & full/half G10 difference $<10^{-2}$ & maximum causal-signature difference $1.733\times10^{-8}$; minimum resolution margin $54.01$ \\
incorrect future branch & $\theta_{(n)}<0$, $\dot{\mathcal A}>0$, $\lambda_0>0$ & all three signs persist in both timestep lanes \\
incomplete flux ledger & charge--flux discrepancy $<1\%$ & maximum $0.302\%$; brane-endpoint subtotal $59.44\%$ of integrated flux \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/tube-evolution.pdf}
\caption{Post-fold evolution of the stable outer branch.  The independently
evolved G10 full- and half-timestep lanes coincide over the nine matched
leaves; open G9 and G11 markers show the common later spatial-transfer
samples.  The one-sided area increases, the least-negative inward expansion
remains below zero, and the principal eigenvalue remains positive.  The
causal-resolution margin is
$\min_{\theta}(|\Xi_{\rm c}|-\delta\Xi)$, where $\delta\Xi$ is the maximum
one-sided stencil spread; it remains positive at every one of 501 nodes and
has minimum $54.01$ in the fixed generator normalization.}
\label{fig:tube-evolution}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/tube-balance.pdf}
\caption{Brane-inclusive balance on the fixed segment
$0.001375\leq t/\ell\leq0.00146875$.  (a) The charge change and integrated
flux lie close to the exact-closure diagonal for G9/G10/G11 and stencil
widths 5, 7, and 9.  (b) Every relative charge--flux discrepancy remains
below the prospective $1\%$ ceiling.  (c) The coupled brane-endpoint subtotal
supplies approximately $59.44\%$ of the integrated flux.  This is a local
ledger decomposition, not a global source attribution.}
\label{fig:tube-balance}
\end{figure}

Figure~\ref{fig:tube-evolution} makes the nine-leaf sampling and its temporal
and spatial comparisons explicit.  The largest stored inward expansion is approximately
$-2.8587\times10^{-2}$, the marginality defect is at most
$1.85\times10^{-6}$ in the interior, and the orthogonal-contact residual is
at most $1.10\times10^{-18}$.  The integrated balance is tested on the fixed
subsegment $0.001375\leq t/\ell\leq0.00146875$.  Its worst adjacent-grid
transfer metric is $7.58\times10^{-5}$ and its worst within-grid spatial-stencil
metric is $3.54\times10^{-5}$.  The brane terms are nonzero and are retained
in the ledger rather than absorbed into the residual.  As shown in
Fig.~\ref{fig:tube-balance}, their subtotal supplies approximately $59.44\%$
of the integrated flux on every grid.  The seam coefficient $c=W_s/W$ is fixed
from the Israel boundary data and the coupled seam law is derived before the
charge--flux residual is examined; neither is fitted to obtain closure.

These outcomes address distinct alternatives: surface persistence alone could
hide a timelike or unresolved tube; area increase alone could be a
finite-difference artifact; and a small bulk-only residual could omit the
moving cap--brane joint.  Their joint closure supports the finite-resolution
interpretation that the fold-selected stable branch develops into a spacelike
free-boundary marginal, or dynamical-horizon, segment.  It does not turn the
segment into an event horizon or a global conserved-charge construction.
Extended values and the boundary-coupled balance derivation are reported in
the Supplemental Material \cite{SupplementalMaterial}.

\section{Interpretation}

The first result is a boundary-sensitive test of the local MOTS bifurcation
...
adjoint projection.  The branches are connected by pseudo-arclength
continuation; an isolated near-critical principal mode accompanies the
bracketed zero crossing; both discrete-adjoint nondegeneracy projections are
nonzero; and an invariant geometric separation has the expected one-half exponent.
...
The second result supplies the physical continuation.  The stable branch does
not merely recur on adjacent slices: the resolved normal generator is
future-outward and spacelike, the inward expansion is negative, the area
increases, and the principal eigenvalue remains positive.  The
brane-inclusive finite-segment balance then connects the changing quasi-local
charge to an explicit bulk-plus-boundary flux ledger.  Spatial transfer,
stencil variation, and an independently evolved half-timestep parent constrain
different numerical explanations for that combined behavior.  In this sense
the fold selects a stable branch that becomes a finite dynamical-horizon
segment rather than an isolated collection of roots.

The concrete significance lies in both the surface class and the dynamical
sequence.  The MOTSs terminate orthogonally on a brane inside an explicitly
stabilized compact interval.  They are neither imposed as static horizons nor
continued from a pre-existing connected black string.  The established
saddle-node mechanism is realized with a physical Robin boundary and is then
followed into a spatially and temporally controlled free-boundary marginal
tube.  That connected sequence, rather than the abstract fold mechanism, is
...
The marginal-tube claim is restricted to nine matched leaves over the tested
finite interval, the same symmetry sector, and the chosen foliation.  Its
reflected quasi-local charge is a boundary-aware finite-segment diagnostic,
not a global mass, source-ownership, or inter-sector transfer ledger.
Finally, no event horizon, future-complete causal boundary, common horizon,
global topology change, or nonlinear stability theorem follows from the evidence.
...
We have followed a dynamically selected free-boundary marginal geometry from
its local creation to a finite spacelike tube segment.  The physical contact
condition enters the nonlinear MOTS map, its Robin domain, and the
endpoint-eliminated discrete adjoint projection.  Across G9, G10, and G11,
the evolved parent changes from no accepted capped surface to two
opposite-stability branches.  Pseudo-arclength continuation joins those
branches at a grid-consistent critical surface; the principal mode crosses
zero; both discrete-adjoint nondegeneracy coefficients are nonzero; and the
invariant area separation follows the expected near-one-half law.  The
independent half-timestep parent reproduces this critical structure.

The stable outer branch then develops into a future-outward spacelike marginal
tube with negative inward expansion, increasing area, positive principal
stability, orthogonal contact, and a brane-inclusive finite-segment balance.
Those properties survive the same three-grid spatial transfer and the
independent G10 half-timestep evolution.  The combined result therefore moves
beyond pair anatomy: it numerically resolves the boundary-sensitive fold and
shows what its stable branch becomes over a controlled post-fold interval.
This supplies a reusable route from free-boundary MOTS bifurcation to local
dynamical-horizon geometry in an evolved higher-dimensional spacetime.  The
result remains finite resolution and quasi-local; no event horizon, global
topology, source ownership, or continuum theorem is inferred.
...
Machine-readable result records and frozen protocol manifests are archived in
the Zenodo version identified by
...
\href{https://github.com/RonBibb/free-boundary-mots-saddle-node/tree/zenodo-22148554}{tagged GitHub source snapshot}
linked from that Zenodo record.  Zenodo is the citable authority for the
saddle-node and marginal-tube result records; the immutable GitHub tag is the
corresponding public code record.
