...
\begin{abstract}
The boundary of a marginally outer trapped surface (MOTS) changes the domain
and adjoint of its stability problem, so closed-surface bifurcation examples do
not by themselves establish that the usual pair-creation normal form survives.
We resolve that normal form for a brane-terminating MOTS pair in the
...
adjoint projections give nonzero near-critical transversality and quadratic
coefficients under a fully specified discrete normalization.  The invariant
...
$p\simeq0.498$.  Together these tests resolve the local saddle-node reduction
for a dynamically generated free-boundary MOTS problem rather than inferring
...
\cite{PaetzSimon2013,AlaeeLesourdYau2020,Chai2024,Mendes2021,Lee2025}.
...
is more general than whether two surfaces appear in one braneworld evolution:
does the local saddle-node reduction persist when the marginal surface has a
...
metric family?  We answer this question in the concrete and demanding case of
brane-terminating free-boundary MOTSs in a compact, explicitly stabilized
braneworld.

The new result is a normal-form structure reproduced across three spatial
grids and in an independent half-timestep evolution in a substantially
...
creation.  Thus we test the reach of the general MOTS bifurcation framework in
a setting where its realization was unresolved, without proposing a new
universal bifurcation class.
...
from the remainder of the reduced-sector spectrum, the adjoint transversality
and quadratic coefficients are nonzero, and the invariant area separation has
...
numerical alternatives.  The central claim is a finite-resolution numerical resolution,
...
&-\sum_i\int_{\Sigma_i}\dd^4x\sqrt{-h_i}
 \,[\lambda_i+U_i(\Phi)],
\qquad U_i(\Phi)=\frac{\gamma_i}{2}(\Phi-v_i)^2.
\label{eq:action}
\end{align}
...
eigenvalue is outward-stable.  The operator and its Robin domain commute with
the $SO(3)$ action.  The elliptic principal eigenfunction is positive and
unique up to scale; applying any group element therefore reproduces the same
...
=\left(\theta_+[\rho;t],\,
\mathcal R_0[\rho;t],\,\mathcal C_B[\rho;t]\right)=0,
\qquad
\mathcal R_0=\rho'(0),\quad
...
brane geometry enters both the kernel and the adjoint problem; it is not an
external endpoint check applied after the stability calculation.
...
reduced map, let $f_R$ span a simple kernel of $D_f\mathcal F$ and let $f_L$
span the one-dimensional adjoint kernel.  With
...
D_f^2\mathcal F[f_R,f_R]\right\rangle.
...
\end{equation}
Consequently, any smooth invariant $Q$ with
$D Q[f_R]\neq0$ separates as
\begin{equation}
Q_+-Q_-
...
principal zero mode, the adjoint coefficients $a$ and $b$, and the one-sided
proper area $Q=\mathcal A$.  Equations~\eqref{eq:free-boundary-normal-form}--
\eqref{eq:invariant-fold-scaling} state the local numerical hypothesis being
tested; they are not asserted here as a new continuum theorem
...
steps are not used to infer an error order.  The continuation, operator, adjoint, and
...
the critical kernel, and the adjoint used in the normal-form projection.  The
branches are
...
the adjoint temporal transversality and quadratic coefficient are nonzero;
...
every free-boundary MOTS problem has the same unfolding.
...
We have numerically resolved the local normal form of a dynamical
free-boundary MOTS saddle-node.  The physical contact condition enters the
boundary-value map and its adjoint problem, while the ambient geometry is
...
a simple critical kernel; the adjoint
...
scripts supporting the figures are archived at Zenodo under
