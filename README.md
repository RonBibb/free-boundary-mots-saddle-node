...
trapped-surface saddle-node in a dynamically evolved five-dimensional,
...
| `protocols/` | Frozen Python source, tests, and protocol descriptions for the load-bearing Protocol 226 and Protocols 228--232 calculations. Numerical `candidate-output` directories and machine-specific authority records are intentionally absent from Git. |
...
The initial repository release contains:
...
- 17 Python files within the frozen protocol directories; and
- the publication figure generator.

That is 543 versioned Python files. All are covered by `MANIFEST.sha256`.
