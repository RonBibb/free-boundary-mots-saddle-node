#!/usr/bin/env python3
"""Independent FD/Newton replication of the equilibrium-GW G4 cap fold."""
...
collocation_fold=8.501562470728416;near=runs[-1]["jacobian_diagnostics"][-1]["branches"]
summary={
...
payload={
    "status":"equilibrium_GW_initial_slice_independent_surface_replication_passed",
    "metric_case":"G4_R8","epsilon":.1,"backreaction_b0":.01,"amplitude_step":.005,
    ...
    "limitations":["same G4 metric solver", "fixed-amplitude normal-form FD fit", "equilibrium-profile initial-data subfamily"],
}
Path("results/gw_slice_capped_fd_replication.json").write_text(json.dumps(payload,indent=2,sort_keys=True)+"\n")
