#!/usr/bin/env python3
"""Pseudo-arclength crossing of the equilibrium-GW G4 capped fold."""
...
config={
    "nz":49,"nr":73,"r_max":8.,"epsilon":.1,"backreaction_b0":.01,
...
def geometry(amplitude,initial=None):
    return solve_gw_slice(
        ...
        epsilon=config["epsilon"],backreaction=config["backreaction_b0"],initial=initial,
...
payload={
    "status":"equilibrium_GW_initial_slice_pseudo_arclength_fold_crossed" if all(item["crossed"] for item in runs) else "incomplete",
    ...
    "limitations":["G4 metric and one domain", "local quadratic turn fit", "no quasi-static delta-Phi solve"],
}
Path("results/gw_slice_capped_arclength.json").write_text(json.dumps(payload,indent=2,sort_keys=True)+"\n")
