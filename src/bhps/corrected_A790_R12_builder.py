"""Task-specific matched-spacing Rmax=12 builder for corrected A=7.90 data.
...
basis on Rmax=12 and solves fresh A=7.90 constraint data.  It never stretches
or extrapolates a smaller-domain solution into the added exterior.
"""
...
def build_A790_R12_base(selector_iterations=35, slice_iterations=280):
    """Build G5-quality Rmax=12 data from its domain-qualified A=8 seed."""
    ...
    selected = solve_anisotropic_initial_data(
        ...
        *shape, reference["background"], chi_r, chi_z,
        initial_q=archive["q_G5R12"], initial_phi=archive["phi_G5R12"],
        stencil_width=7, tolerance=1e-9, iterations=int(selector_iterations),
    )
    return _assemble(reference, selected, shape, AMPLITUDE, "G5A790R12")


def build_A790_R12_refined(
    coarse, nz, nr, name, selector_iterations=45, slice_iterations=320,
):
    """Refine within Rmax=12 using only a same-domain interpolated seed."""
    nz = int(nz)
    nr = int(nr)
...
def build_A790_R12_pair():
    base = build_A790_R12_base()
    g7 = build_A790_R12_refined(
        base, 81, matched_radial_count(121), "G7A790R12",
        selector_iterations=45, slice_iterations=320,
    )
    g8 = build_A790_R12_refined(
        g7, 97, matched_radial_count(145), "G8A790R12",
