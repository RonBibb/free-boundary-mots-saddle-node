#!/usr/bin/env python3
"""Robustness to controlled nonzero initial stabilizer acceleration."""
...
cases=[
    {"id":"mode1_minus5","mode":1,"strength":-5.},
    {"id":"stationary","mode":1,"strength":0.},
    {"id":"mode1_plus5","mode":1,"strength":5.},
    {"id":"mode2_minus5","mode":2,"strength":-5.},
    {"id":"mode2_plus5","mode":2,"strength":5.},
]
...
for case in cases:
    ...
    for amplitude in np.arange(8.70,8.45,-.005):
...
baseline=next(item for item in outputs if item["id"]=="stationary")
payload={
    "status":"capped_fold_persists_under_controlled_nonzero_initial_stabilizer_acceleration",
    "metric_case":"G4_R8","wall_stiffness":20.,"cases":outputs,
    ...
    "interpretation":"For lapse N=psi and Pi_Phi=0, the sourced selector F=S corresponds in the bulk to initial scalar acceleration partial_t Pi_Phi=S/psi.",
    "limitations":[
        "single G4 grid/domain","two prescribed acceleration shapes and finite strength samples",
        "normal-form fold fits","higher-order evolution-boundary compatibility not adjudicated",
        "not a full dynamical evolution",
    ],
}
Path("results/stabilizer_acceleration_robustness.json").write_text(json.dumps(payload,indent=2,sort_keys=True)+"\n")
print(json.dumps({
